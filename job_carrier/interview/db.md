...
+ [Аномалии сериализации. Какие проблемы могут возникать при параллельном доступе с использованием транзакций?](#Аномалии-сериализации.Какие-проблемы-могут-возникать-при-параллельном-доступе-с-использованием-транзакций?)
+ [Механизмы блокировок](#Механизмы-блокировок)
...
## Что такое _«реляционная база данных»_?
__Реляционная база данных__ — база данных, в которой данные организованы в виде связанных таблиц, также называемых отношениями.
...
_Нормализация_ - это процесс преобразования базы данных к минимальной избыточности (в данных и структуре).
__Цель__: 
* устранение аномалий
* повышение производительности
* повышения удобства управления данными

__Избыточность данных__ - это состояние, при котором одни и те же данные хранятся в нескольких местах БД. Это приводит к аномалиям, так как при управлении над данными, нужно вносить изменения в нескольких местах. Чем выше NF базы данных, тем меньше в ней возможный аномалий (Рекомендуемая NF БД - 3, выше уже по предпочтению).

[к оглавлению](#Базы-данных)

## Какие существуют нормальные формы?
__Нулевая нормальная форма__(UNF)
* Строки не должны быть пронумерованы (номер записи не хранится в таблице).
* Порядок строк и столбцов не имееет значения

__Первая нормальная форма (1NF)__ (любая реляционная БД) - Отношение находится в 1NF, если значения всех его атрибутов атомарны (неделимы). 
* Все строки в таблице уникальны
* В каждой ячейке таблцы хранится атомарное значение
* В столбце хранятся данные одного типа
* Отсутствуют массивы и списки в любой виде
...
* Наличие у всех таблиц ключа (для точной идентификации строки любой таблицы)
* Если ключ составной, то все его неключевые столбцы должны зависеть от полного ключа
* Достигается созданием ключей и разбиеним таблицы на несколько таблиц, одна из которых линковочная
...
* Достигается за счёт декомпозиции аномальных таблиц на таблица А + таблица допустымых значений B для колонки(ок) таблицы А (ссылаемся на значения из другой таблицы, например, справочники)

__Транзитивная зависимость__ - зависимость, при которой неключевые столбцы зависят от значений других неключевых столбцов.
...
* Отсутствие нетривиальных многозначных зависимостей
Пример: в таблице есть колонки: Курс | ФИО преподавателя | Аудитория.
При этом Курс может вести не обязательно только один преподаватель. Курс не всегода может проходить только в одной аудитории. Значит наличие колонок Преподаватель и Аудитория в одной таблице создаёт многозадачную зависимость.
Решение: Разделение таблицы на две: Курс | ФИО Преподавателя и Курс | Аудитория.

__Почему достаточно наличия 4NF?__:
* Остальные формы используются в ~1% случаев.
* Нормализация повышает производительность дл 3-4 NF, затем наоборот снижается.
...
* Наличие ограничения доменов
* Наличие ограничения ключей

__Ограничение домена__ - соблюдение для некоторого атрибута значений, только из некоторого заданного домена (набора значений)
__Ограничение ключа__ - соблюдение, что некоторый атрибут (или их комбинация) представляет собой потенциальный ключ.
...
__Индекс (index)__ — объект БД, используемый для повышения производительности(оптимизации) запросов (ассоциация - содержание книги). 
...
Критерии условия запроса (Where) по колонке с индексом будет работать значительно быстрее. Оптимизатор запроса будет искать наиболее эффективный способ выполнения запроса.
При поиске по полю с индексом используется бинарный(двоичный) поиск.
...
+ Полей, которые содержат всего два или три значения или NULL, например: _мужской_,  _женский пол_ или значения _«да»_, _«нет»_;
+ Таблиц, с частыми массовыми вставками и обновлениями.
...
__По воздействию на источник данных__ (основное деление)
...
__В чем отличие между кластерными и некластерными индексами?__
...
(Некластерилизованный индекс хранится в одном месте, а физические данные таблицы - в другом (выполняются меделеннее кластерилизованных)).

При кластерном индексировании данные физически упорядочены, что серьезно повышает скорость выборок данных (но только в случае последовательного доступа к данным). Для одного набора данных может быть создан только один кластерный индекс (использует PK для структуризации данных в таблице. Создаётся по умолчанию при определении ключа).

__По порядку сортировки__: упорядоченные, неупорядоченные, возрастающие, убывающие.
__По источнику данных__: индексы по представлению (view), индексы по выражениям.
__По структуре__: B*-деревья, B+-деревья, B-деревья, Хэши.
...
__Транзакция__ - это упорядоченное множество операций, переводящих БД из одного состояния в другое. 
* Транзацкия работает как архив запросов (атомарный набор операций) по принципу "всё или ничего".
* В рамках одного соединения НЕ может проходить несколько транзакций параллельно (соединение рассматривается как труба, в которую помещяется только 1 транзакция).
* Лучше использовать 1 соединение, так как его открытие - ресурснозатратная операция.
* Транзакцию можно открыть (например командой START TRANSACTION), а затем подтвердить - COMMIT или откатить - ROLLBACK.
* Пока транзакция не будет применена, запрос (в рамках этой транзакции) выполняется виртуально (реально ничего не применяется).
* Транзакция обязательно должны быть применена или отклонена, так как с момент старта транзакции данные заблокированы.
...
## Аномалии сериализации. Какие проблемы могут возникать при параллельном доступе с использованием транзакций?
Аномалии сериализации возникают, когда результат параллельного выполнения транзакций не эквивалентен **ни одному** из возможных последовательных (сериальных) исполнений.
...
__Блокировки__ - основной метод реализации изоляции в большинстве СУБД.
Типы блокировок:
- __Разделяемая (Shared, S-lock)__ — для операций чтения. Несколько транзакций могут одновременно удерживать S-блокировку на одном объекте.
- __Монопольная (Exclusive, X-lock)__ — для операций записи. Только одна транзакция может удерживать X-блокировку.

__Протоколы блокировок__: 

Двухфазная блокировка (2PL) - гарантирует сериализуемость.
- Фаза роста — транзакция только приобретает блокировки.
- Фаза сокращения — транзакция только освобождает блокировки.
- Строгий 2PL — все X-блокировки удерживаются до конца транзакции (commit/rollback). Используется чаще всего.

__Проблемы блокировок__:
* Взаимная блокировка (Deadlock) - циклическая зависимость блокировок между транзакциями (решение: предотвращение deadlock или обнаружение и откат одной из транзакций).
* Голодание (Starvation) - Транзакция долго не может получить нужную блокировку из-за потока других транзакций.

### Альтернатива блокировкам
Оптимистический контроль параллелизма - это альтернатива блокировкам (для сценариев с редкими конфликтами), суть которой проверка конфликтов только при фиксации транзакции.
__Этапы__:
1. Чтение: данные считываются без блокировок, сохраняются их версии.
2. Модификация: изменения делаются в локальной копии.
3. Валидация: при фиксации проверяется, не менялись ли данные другими транзакциями.
4. Запись: если конфликтов нет — запись, иначе откат.

[к оглавлению](#Базы-данных)

# Источники
+ [Википедия](https://ru.wikipedia.org/wiki/)
+ [GitHub java-interview](https://github.com/enhorse/java-interview/blob/master/README.md)
+ [YouTube chanel ListenIT]
+ 
