...
### Отличие функций от представлений
__Хранимые процедуры и функции__ — это заранее написанные и сохранённые в базе данных SQL-скрипты, которые можно вызывать по имени.

__Для чего используют__:
...
* Процедура может не возвращать или возвращать несколько значений, в то время как ФУНКЦИЯ всегда возвращает одно значение.
* Процедура вызывается отдельно, в то время как ФУНКЦИЮ можно использовать в SELECT, WHERE и других частях запроса.
* Процедура может изменять данные в таблицах, а ФУНКЦИЯ нет - предназначена только для чтения данных.
